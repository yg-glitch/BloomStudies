...
export default function CommunityPage() {
  ...
  const [user, setUser] = useState<CommunityUser>(generateMockUser()) // Temp default
    ...
    async function loadData() {
      ...
      if (profile) {
        setUser({
          ...
          xp: profile.xp || 0,
          level: profile.level as any || 'Seedling',
      ...
      setPosts(dbPosts.map((p: any) => ({
        ...
        author: {
          ...
          level: p.profiles?.level as any || 'Seedling',
          badges: [],
  ...
  const handlePost = async () => {
    ...
    try {
      const dbPost = await import('@/lib/database/community').then(m => m.createCommunityPost(user.id, {
      ...
      toastSuccess('Error', 'Failed to publish post')
    } finally {
      setIsPosting(false)
    }
  }
