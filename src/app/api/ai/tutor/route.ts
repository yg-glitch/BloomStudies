...
export async function POST(req: NextRequest) {
  try {
    ...
    // Try to save to DB if user is logged in (non-blocking)
    let activeConversationId = conversationId
    try {
      const supabase = await createClient()
      const { data: { user } } = await supabase.auth.getUser()
      ...
      if (user) {
        ...
        if (!activeConversationId) {
          ...
          const { data: conv } = await supabase
            .from('conversations')
            .insert({ user_id: user.id, title, subject: subject || null, education_system: educationSystem || 'leaving-cert' })
            .select()
            .single()
          activeConversationId = conv?.id
        }
      ...
      // DB save failure is non-blocking — AI still responds
    }
    ...
    // Stream response and save AI reply to DB
    let fullResponse = ''
    const encoder = new TextEncoder()
    const stream = new ReadableStream({
      async start(controller) {
...
function buildSystemPrompt(subject?: string, level?: string, educationSystem?: string, action?: string): string {
  ...
  if (action === 'quiz') {
    ...
    prompt += `\n\nExplain at Higher Level standard — precise academic language, depth, edge cases, connections to other curriculum topics.`
  }
